...
int predicteur(int nH, int nW, OCTET* Img, int i, int j){
	if(i >0 and j> 0){
		return 	( Img[(i-1)*nW+j] +  Img[(i-1)*nW+j-1] + Img[i *nW+j-1] )/ 3;
	}
	else
		return Img[i*nW+j];
}

int inverse(int nH, int nW,OCTET* Img, int i, int j){
	if(Img[i*nW+j]>=128)
		return Img[i*nW+j]-128;
